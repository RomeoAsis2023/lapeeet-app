...
## Corpus Check
- 23 files · ~39,925 words
- Verdict: corpus is large enough that graph structure adds value.
- Unclassified: 8 file(s) not represented in the graph (top: .css 5, (none) 2, .psd 1)

## Summary
- 475 nodes · 834 edges · 28 communities (27 shown, 1 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 61 edges (avg confidence: 0.93)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `a7f2273f`
...
6. `_render()` - 12 edges
7. `sendDirect()` - 12 edges
8. `Lapeeet — Development Phases (Single-File Live P2P)` - 12 edges
9. `_run()` - 11 edges
...
### Community 1 - "app.js"
Cohesion: 0.08
Nodes (56): Phase 6 — `?mode=` Role Deep Links ✅ DONE, PHASE 0 — Project Scaffolding & Static Setup — **100% ✅ COMPLETED**, PHASE 1 — Map Layer + 60 km Cap + Organic Maps Deep Links — 🚧 PARTIALLY STARTED, _bindAutocomplete(), _bindCapacityDropdown(), applyFocus(), open(), _bindDataScreen() (+48 more)

### Community 2 - "db-layer.js"
Cohesion: 0.08
Nodes (49): Lapeeet — dev_phases.md (master plan, no skips), Live vs Dev separation (locked), Open decisions (need your call), Phase 10 — Rich Profile (avatar + first/last + email) ✅ DONE, Phase 2 — sql.js Tenant DB + Onboarding + E-Bikes ✅ DONE, Phase 3 — webconnect.js P2P Ride Flow ✅ DONE (code; field test pending), Phase 5 — Polish + Manifest ✅ DONE (code; device test pending), Phase 7 — GitHub Pages Deploy ✅ LIVE (+41 more)
...
Cohesion: 0.19
Nodes (22): Phase 9 — Phone Registration Gate + WebAuthn Passkeys ✅ DONE (code; ceremony field test pending), b64urlDecode(), b64urlEncode(), cborDecode(), item(), read(), uint(), coseToSpki() (+14 more)
...
## Knowledge Gaps
- **120 isolated node(s):** `fs`, `vm`, `src`, `sandbox`, `raw` (+115 more)
...
- **Why does `Lapeeet — dev_phases.md (master plan, no skips)` connect `db-layer.js` to `app.js`, `map-layer.js`, `build-single.py`, `call-layer.js`, `base.js`, `auth-layer.js`?**
  _High betweenness centrality (0.403) - this node is a cross-community bridge._
- **Why does `Phase 9 — Phone Registration Gate + WebAuthn Passkeys ✅ DONE (code; ceremony field test pending)` connect `auth-layer.js` to `app.js`, `db-layer.js`?**
